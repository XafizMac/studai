...
export default class Store {
  ...
  me: {
    ...
    lastName: string;
    photo: string;
  } = { id: 0, email: "", firstName: "", lastName: "", photo: "" };
  isAuth = false;
  ...
  async registration(
    ...
    try {
      ...
      const status = (e as any).response?.status || 500;
      const statusText =
        (e as any).response?.data?.email[0] || "Неизвестная ошибка";
      this.setStatus(status, statusText);
      return { status, statusText };
    }
  }
  ...
  async generatePlan(
    ...
    try {
      const response = await AuthService.generatePlan(
        workType,
        languageOfWork,
        workTheme,
        discipline,
  ...
  async regeneratePlan(
    ...
    try {
      const response = await AuthService.regeneratePlan(
        id,
        workType,
        languageOfWork,
        workTheme,
  ...
  async resetPassword(email: string) {
    try {
      const response = await axios.post(`${API_URL}/users/reset_password/`, {
        email,
      });
      this.setStatus(response.status, response.statusText);
      console.log(response);
